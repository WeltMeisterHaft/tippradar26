...
function isPlaceholderTeam(name) {
  const value = normalizedTeamName(name || "");
  return !value
    || value.includes("noch offen")
    || value.includes("teilnehmer spiel")
    || value.includes("winner")
    || value.includes("gewinner")
    || value.includes("loser")
    || value.includes("verlierer")
    || value.includes("tbd")
    || value.includes("unknown")
    || value.includes("to be defined")
    || /^([123]\.|[123]\s)/.test(value);
}
...
function projectedKnockoutMatches(tips = collectTips()) {
  ...
  knockoutPhases.forEach((phase) => {
    ...
    for (let index = 0; index < phase.count; index += 1) {
      ...
      const officialEntries = officialKnockoutEntries(official);
      const homeEntry = officialEntries?.home || pairing[0];
      const awayEntry = officialEntries?.away || pairing[1];
...
function simulateKnockoutMatch(home, away, matchId, official = null) {
  const officialEntries = officialKnockoutEntries(official);
  if (officialEntries) {
    const homeEntry = officialEntries.home;
    const awayEntry = officialEntries.away;
    if (official.result) {
  ...
  home ||= { team: "Noch offen" };
  away ||= { team: "Noch offen" };
...
function renderTournamentSimulation() {
  ...
  const actualKnockoutGames = [...roundOf32, ...roundOf16, ...quarterFinals, ...semiFinals, ...final]
    .filter((match) => match.actual).length;
  const baseText = `Kombi-Simulation: ${actualGroupGames + actualKnockoutGames} IST-Ergebnisse, offene Spiele nach ${modelNames[simulationModel]}.`;
  status.textContent = simulationModel === "own" && missing
    ? `${baseText} ${simulation.predicted} von ${simulation.total} Gruppenspielen berücksichtigt. Noch ${missing} Gruppentipps fehlen.`
    : `${baseText} Alle ${simulation.total} Gruppenspiele sind in der Was-wäre-wenn-Projektion berücksichtigt.`;
}
